...
def skrot(message):
    ...
    return "".join(chr(65 + value % 26) for value in result)
...
def task_78_1(answer_filename):
    ...
    message_hash = skrot(message)
    hash_values = [ord(v) for v in message_hash]
    
    with open(answer_filename, "a") as f:
        f.write("Zadanie 78.1\n")
        f.write(f"a) {filled_length}\n")
        f.write(f"b) {' '.join(str(v) for v in hash_values)}\n")
